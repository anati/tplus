...
// Custom descriptors for Talent Plus.
$custom_labels = array(
    'News Room' => array('singular' => 'News Room Article', 'plural' => 'News Room Articles'),
    'Audio | Video' => array('singular' => 'Audio | Video File', 'plural' => 'Audio | Video Files'),
    );
?>
...
<tr>
    ...
    <?php 
    // Display 'books', 'press releases', etc. instead of "articles."
    if( isset( $custom_labels[$category->title] ) )
    {
        $label = $custom_labels[$category->title];
    }
    else
    {
        $label = array(
            'singular'  => preg_replace('/s\s*$/', '', $category->title), 
            'plural'    => $category->title
            );
    }
    ?>
		...
		<li>
			...
			<span class="small">
                ...
                if ($category->numitems==1) 
                    {
    				    echo $category->numitems ." ". $label['singular'];
    				}
                    else
                    {
    				    echo $category->numitems ." ". $label['plural'];
                    }
                echo ")";
             ?>
			</span>
