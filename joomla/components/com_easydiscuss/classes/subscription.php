	...
	function process()
	{
		...
		foreach($subscribers as $subscriber)
		{
			...
			if( $rows )
			{
				foreach( $rows as $row )
				{
					$row['link']			= DiscussRouter::getRoutedURL('index.php?option=com_easydiscuss&view=index&category_id='.$row['category_id'], false, true);
					$row['categorylink']	= DiscussRouter::getRoutedURL('index.php?option=com_easydiscuss&view=post&id='.$row['id'], false, true);
					...
					$row['message']		= Parser::bbcode( $row['content'] );
