...
class DNotification
{
	...
	function addQueue($toEmails, $subject = '', $body = '', $template='', $data = array())
	{
		...
		$mailfrom	= $config->get( 'notification_sender_email' , JFactory::getConfig()->getValue( 'mailfrom' ) );
		$fromname	= $config->get( 'notification_sender_name' , JFactory::getConfig()->getValue( 'fromname' ) );
	...
	function getEmailTemplateContent( $template, $data )
	{
		...
		if( $config->get( 'notify_html_format' ) )
		{
			$output 	= $theme->fetch( 'email.template.html.php' );
		}
		else
		{
			$output 	= $theme->fetch( 'email.template.text.php' );
		}
