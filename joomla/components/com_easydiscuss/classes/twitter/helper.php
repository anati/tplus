...
require_once( DISCUSS_CLASSES . DS . 'twitter' . DS . 'consumer.php' );
...
class DiscussTwitter extends TwitterOAuth
{
	 ...
	 * Process message
	 **/
	function processMessage( $message , $post )
	{
		$search		= array();
		$replace	= array();
			...
			for($i = 0; $i < count($tmp); $i++)
			{
				...
				if(($tempLength > $balance))
				{
					...
					else
					{
						if($balance < 6)
						{
							$temp = JString::substr($temp, 0, $balance);
						}
						else
						{
							$temp = JString::substr($temp, 0, $balance - 3);
							$temp .= ' ...';
						}
