		...
		<?php 
			...
			foreach ($items as $i => &$item)		{
				...
				if ($item->access <= $user->get('aid', 0)) {
                    $titleOverflowClass = strlen($item->name) > 25? " titleOverflow" : "";
					if ($item->content) {
					...
					elseif ($item->name != "") {
						...
						echo '<li class="maximenuCK'.$item->classe.' level'.$item->sublevel.' '.$item->liclass.'" style="z-index : '.$zindex.';">';
								switch ($item->type) :
									default:
										echo '<a class="maximenuCK '.$item->anchor_css.$item->anchor_title.'" href="'.$item->link.'"'.$item->rel.'><span class="titreCK'.$titleOverflowClass.'">'.$item->name.$item->desc.'</span></a>';
										break;
									case 'separator':
										echo '<span class="separator '.$item->anchor_css.'"><span class="titreCK'.$titleOverflowClass.'">'.$item->name.$item->desc.'</span></span>';
										break;
									case 'url':
									case 'component':
										switch ($item->browserNav) :
											default:
											case 0:
												echo '<a class="maximenuCK '.$item->anchor_css.'" href="'.$item->link.'"'.$item->rel.$item->anchor_title.'><span class="titreCK'.$titleOverflowClass.'">'.$item->name.$item->desc.'</span></a>';
												break;
											case 1:
												// _blank
												echo '<a class="maximenuCK '.$item->anchor_css.'" href="'.$item->link.'"'.$item->rel.$item->anchor_title.' target="_blank"><span class="titreCK'.$titleOverflowClass.'">'.$item->name.$item->desc.'</span></a>';
												break;
											case 2:
												// window.open
												//$attribs = 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,'.$this->_params->get('window_open');
												echo '<a class="maximenuCK '.$item->anchor_css.'" href="'.$item->link.'&tmpl=component" onclick="window.open(this.href,\'targetWindow\',\'$attribs\');return false;"'.$item->rel.$item->anchor_title.'><span class="titreCK'.$titleOverflowClass.'">'.$item->name.$item->desc.'</span></a>';
												break;
										endswitch;
										break;
								endswitch;
