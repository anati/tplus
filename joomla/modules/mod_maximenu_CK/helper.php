    ...
    function GetMenu(&$params) {
        ...
        foreach ($items as $i => &$item) {
            ...
            if ($dependantitems && $startlevel && (
                ...
                //if ($startlevel && !in_array($item->parent, $path)) {
                unset($items[$i]);
                continue;
            }
        ...
        foreach ($items as $i => &$item) {
            ...
            if (($menu_image <> '-1') && $menu_image) {
                if ($params->get('menu_images_align') == 1) {
                    $item->image = '<img src="' . JURI::base(true) . '/images/stories/' . $menu_image . '" alt="' . $item->name . '" align="right" />';
                } else {
                    $item->image = '<img src="' . JURI::base(true) . '/images/stories/' . $menu_image . '" alt="' . $item->name . '" align="left" />';
                }
            }
            // if only image, then no title
            if (preg_match('/\[img\]/', $item->name)) {
                $item->image = '<img src="' . JURI::base(true) . '/images/stories/' . $menu_image . '" border="0" alt="' . $item->name . '" align="left" />';
                $titreCK = 'img';
                $item->img = true;
            }
