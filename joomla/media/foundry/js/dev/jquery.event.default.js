...
triggerAsync = function(type, data, success, prevented){
	...
	if ( this[0] ) {
		...
		Foundry.event.trigger( {type: type, _success: success}, data, this[0]  );
	} else{
		success.call(this);
	}
	return this;
...
$event.special["default"] = {
	...
	triggerDefault : function(event, elem){
		...
		var defaultGetter = Foundry.Event("default."+event.type);
...
$._data = function(elem, name, data){
	...
	if(!data && name === "handle"){
		...
		return function(e){
			...
			return typeof Foundry !== "undefined" && (!e || Foundry.event.triggered !== e.type) ?
				Foundry.event.handle.apply( this, arguments ) :
				undefined;
		}
	}
...
$event.trigger =  function defaultTriggerer( event, data, elem, onlyHandlers){
	...
	event = typeof event === "object" ?
		// jQuery.Event object
		event[ Foundry.expando ] ? event :
		// Object literal
		new Foundry.Event( type, event ) :
		// Just the event type (string)
		new Foundry.Event( type );
