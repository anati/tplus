(function( $ ) {
	...
	var bind = function( el, ev, callback ) {
		...
		shifter = function shifter(cb) {
			return function() {
				...
				//!---JSONRMSHACK---!//
				// Check for plUpload object
				// and return arguments in accord to plUpload API
				// TODO: Need a more "general-purpose" way of doing this.
				if (window.plupload)
				{
					if (this instanceof plupload.Uploader)
					{
	 ...
	 */
	{
		 ...
		 * @codeend
		 */
		init: function( baseClass, fullName, staticProps, protoProps ) {
		...
		// !-- JSONRMSHACK --! //
		initInstance: function( proto ) {

			// cache the underscored names
			this._fullName = underscoreAndRemoveController(this.fullName);
			this._shortName = underscoreAndRemoveController(this.shortName);

			// TODO: jQuery plugin support
		...
		_action: function( methodName, options ) {
			...
			// Add the ability to bind an event to multiple elements.
	 ...
	 */
	{
		...
		setup: function( element, options ) {
			...
			// Initialize class only when our prototype is ready,
			// then pass in the prototype as a "self" keyword.
			if (this.Class.initLater)
			{
				$.extend( true, this, this.Class.initLater[3].apply( this.Class, [this]) );
				this.Class.initInstance.apply(this.Class, [this]);
			}
			...
			// Augment selector properties "{}" provided in the options
			// into selector functions within the instance of the class.
			...
			for ( funcName in cls.actions ) {
				if ( cls.actions.hasOwnProperty(funcName) ) {
					...
					{
						_this._bindings.push(
							ready.processor(ready.delegate || element, ready.parts[2], ready.parts[1], _this.callback(funcName), _this)
						);
					});
				}
			}
		...
		find: function( selector ) {
			if (selector.indexOf("{")==0)
				selector = this.options[selector.substr(1,selector.length-2)];
