...
{
	window.location.href	= 'index.php?option=com_easydiscuss&view=rules&from=points';
}
		...
		<table class="adminlist" cellspacing="1">
			<thead>
				<tr>
					...
					<th width="5"><input type="checkbox" name="toggle" value="" onClick="checkAll(<?php echo count( $this->points ); ?>);" /></th>
			...
			if( $this->points )
			{
				...
				for ($i=0, $n = count( $this->points ); $i < $n; $i++)
				{
					...
					$editLink	= JRoute::_( 'index.php?option=com_easydiscuss&view=point&id=' . $row->id );
				?>
