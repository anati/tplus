...
class EasyDiscussAdminView extends JView
{
	function &getModel( $name = null )
	{
		...
		if( !isset( $model[ $name ] ) )
		{
			$path	= DISCUSS_ADMIN_ROOT . DS . 'models' . DS . JString::strtolower( $name ) . '.php';
	...
	function renderCheckbox( $configName , $state )
	{
		ob_start();
	?>
		<label class="option-enable<?php echo $state == 1 ? ' selected' : '';?>"><span><?php echo JText::_( 'COM_EASYDISCUSS_YES_OPTION' );?></span></label>
		<label class="option-disable<?php echo $state == 0 ? ' selected' : '';?>"><span><?php echo JText::_( 'COM_EASYDISCUSS_NO_OPTION' ); ?></span></label>
