...
class EasyDiscussModelReports extends JModel
{
	/**
	 * Blogs data array
	 *
	...
	function _buildQuery()
	{
		...
		$query	= 'SELECT a.*, COUNT(b.`id`) as `reportCnt`, MAX(b.`created`) as `lastreport`, b.`reason`';
		$query  .= ' FROM `#__discuss_posts` AS a ';
		$query  .= ' LEFT JOIN `#__discuss_reports` AS b';
		$query  .= ' ON a.id = b.post_id';

		$query	.= $where;
		$query  .= ' GROUP BY (b.`post_id`)';
		$query	.= ' ' . $orderby;
	...
	function publish( $posts = array(), $publish = 1 )
	{
		if( count( $posts ) > 0 )
		{
			...
			$query	= 'UPDATE ' . $db->nameQuote( '#__discuss_posts' ) . ' '
					...
					. 'WHERE ' . $db->nameQuote( 'id' ) . ' IN (' . $posts . ')';
			$db->setQuery( $query );
	...
	function removePostReports($postId)
	{
	    ...
	    {
		    //now update post record
		    $query  = 'DELETE FROM `#__discuss_posts` WHERE `id` = ' . $db->Quote($postId);
		    $db->setQuery($query);
		    if(! $db->query())
		    {
				$this->setError($this->_db->getErrorMsg());
