...
class EasyDiscussControllerAcl extends EasyDiscussController
{
	function apply()
	{
		...
		$mainframe->redirect( 'index.php?option=com_easydiscuss&controller=acl&task='.$task.'&cid='.$cid.'&type='.$type , $result['message'] , $result['type'] );
	}

	function cancel()
	{
		$mainframe	= JFactory::getApplication();
		$mainframe->redirect( 'index.php?option=com_easydiscuss&view=acls' , $result['message'] , $result['type'] );
	}
			...
			foreach( $bloggers as $id )
			{
				...
				if(!empty($ruleset->id))
				{
					if( !$model->deleteRuleset($id, 'assigned') )
					{
						...
						$type		= 'error';
						$mainframe->redirect( 'index.php?option=com_easydiscuss&view=acls' , $message , $type );
						return;
					}
				}
			}
