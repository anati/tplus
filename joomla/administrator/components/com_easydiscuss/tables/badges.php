...
class DiscussBadges extends JTable
{
	...
	public function bindImage( $elementName )
	{
		...
		$name		= md5( $this->id . JFactory::getDate()->toMySQL() ) . $image->getExtension();

		// @task: Create the necessary path
		$path				= $storage . DS . $this->id;
		...
		JFile::copy( $file['tmp_name'] , $path . DS . $name );

		// @task: Resize to the 16x16 favicon
		$image->resize( DISCUSS_BADGES_FAVICON_WIDTH , DISCUSS_BADGES_FAVICON_HEIGHT );
		$image->save( $path . DS . 'favicon_' . $name );

		$this->avatar		= $this->id . DS . $name;
		$this->thumbnail	= $this->id . DS . 'favicon_' . $name;
