...
class plgSystemEasyCalcCheckPlus extends JPlugin
{
    ...
    public function onBeforeStoreUser($user, $isnew)
    {
        ...
        if(($this->params->get('user_reg') AND $option == 'com_user') OR ($this->params->get('virtuemart') AND $option == 'com_virtuemart') OR ($this->params->get('communitybuilder') AND $option == 'com_comprofiler') OR ($this->params->get('alpharegistration') AND $option == 'com_alpharegistration') OR ($this->params->get('jomsocial') AND $option == 'com_community'))
        {
            if($this->performChecks())
            {
                return true;
